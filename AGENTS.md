...
このリポジトリは現在、設計段階にある。アプリケーション、Docker Compose、Makefile、CI/CDはまだ実装されていない。存在しないコマンドや構成を実装済みとして扱わない。
...
以下は今後Makefileを実装した後の標準コマンドであり、現時点では利用可能とは限らない。
...
コマンド実装後は、AGENTS.mdを実際の引数、前提条件、挙動に合わせて更新する。
